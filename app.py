...
with tab2:
    ...
    selected_date = st.date_input("Pilih Rentang Tanggal", [min_date, max_date])

    # Filter data
    filtered = data[(data['Date'].dt.date >= selected_date[0]) &
                    (data['Date'].dt.date <= selected_date[1])]
...
with tab3:
    ...
    with col2:
        st.metric("Wilayah Risiko Tinggi",
                 latest_data[latest_data['Risk Level'] == "Tinggi"].shape[0])
    with col3:
        st.metric("Wilayah Risiko Rendah",
                 latest_data[latest_data['Risk Level'] == "Rendah"].shape[0])

    # Show table
    st.dataframe(latest_data[['Location', 'Cluster', 'Risk Level',
                            'Total Cases', 'Total Deaths']].sort_values('Risk Level'),
                height=300)
